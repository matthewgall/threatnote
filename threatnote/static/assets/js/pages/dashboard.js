...
var KTDashboard = function() {
    ...
    var widgetSalesStatisticsChart = function() {
    	if (!document.getElementById('report_intel_data')) {
	        var REPORTS_DATA=[20, 30, 40, 35, 45, 55, 45];
	        var INTEL_DATA=[25, 35, 45, 40, 50, 60, 50];
	        var CUSTOMERS= ['Finance', 'IT', 'SOC', 'IR', 'HR', 'Executives'];
	        return widgetSalesStatisticsChartNew(CUSTOMERS, REPORTS_DATA, INTEL_DATA, 'kt_chart_sales_statistics');
    	} else {
    		return;
    	}
    }
    var widgetReportIntelChart = function () {
    	if (!document.getElementById('report_intel_data')) {
            return;
        }
        //var REPORTS_DATA=[20, 30, 10, 35, 45, 55, 45];
        //var INTEL_DATA=[4, 35, 45, 40, 50, 60, 50];
        //var CUSTOMERS= ['X', 'YT', 'SOC', 'IR', 'HR', 'E'];
        var REPORTS_DATA=[];
        var INTEL_DATA=[];
        var CUSTOMERS= [];

    	var DATA_LIST=document.getElementById('report_intel_data').children;
    	
    	for(i=0; i< DATA_LIST.length; i++) {
    		
    		if(DATA_LIST[i].id.includes('data_label')) {
        		CUSTOMERS.push(DATA_LIST[i].textContent);
        		//console.log('data_label'+i+' '+DATA_LIST[i].textContent);
    		}
    		if(DATA_LIST[i].id.includes('data_report')) {
        		REPORTS_DATA.push(parseInt(DATA_LIST[i].textContent));
        		//console.log('data_report'+i+' '+DATA_LIST[i].textContent);
    		}
    		if(DATA_LIST[i].id.includes('data_intel')) {
        		INTEL_DATA.push(parseInt(DATA_LIST[i].textContent));
        		//console.log('data_intel'+i+' '+DATA_LIST[i].textContent);
    ...
    var widgetSalesStatisticsChartNew = function(labels, dataset1, dataset2, divname) {
        ...
        var barChartData = {
//            labels: ['Finance', 'IT', 'SOC', 'IR', 'HR', 'Executives'],
        		labels: labels,
        		datasets: [{
                label: 'Reports',
